...
```javascript
{
  ...
  genderScoring: {                   // number: gender-specific linear scoring
    male: { inputMin, inputMax },
    female: { inputMin, inputMax }
  },
...
**Single source of truth: `studies.json`** contains all study data in four collections: `studies` (full bibliographic records), `claims` (claim tag vocabulary), `study_claims` (study↔claim links with direction +/−/±), and `evidence_usage` (study↔evidence-page-card links). Managed via `registry.py`. This is public scientific data — no personal or health information.

**Study ID resolution:** Evidence page JSONs (e.g., `sleep.json`) reference studies by ID (e.g., `"windred-2024"`) instead of embedding HTML strings. At build time, `evidence-builder.py` resolves each ID from `studies.json` and generates the full citation HTML (badge, title, metadata, key finding, DOI link). This eliminates duplication and ensures citation data stays consistent across all pages. The builder is backward-compatible: if a value contains HTML (`<` or `>`), it passes through unchanged, allowing gradual migration per category.

**Key commands:** `registry.py query "claim-tag"` (find studies), `registry.py summary` (Claim Summary Index), `registry.py stale` (overdue verifications), `registry.py add` (new study), `registry.py export-summary` (update study-registry.md), `registry.py stats` (overview), `registry.py gaps` (claims needing more studies), `registry.py enrich-all` (studies with missing fields), `registry.py export-xlsx [CATEGORY]` (formatted Excel export), `registry.py supersede OLD NEW` (mark study replaced).
...
| `number` (genderScoring) | Gender-specific linear: uses `genderScoring[sex].inputMin/Max`, same formula |
...
**Citation format in JSON:** Use study IDs (e.g., `"windred-2024"`) in `studyCitations` and `studyRefs` arrays. The builder resolves them from `studies.json` at build time. Legacy HTML strings are still supported for categories not yet migrated. Migrated: sleep.

**Expert citation format in JSON:** Use `expertCitations` array on each card. Each entry has `expert` (name), `work` (book/chapter), and `publicUrls` (array of links). Each URL object has `url`, `label`, and `source` (displayed as an italic heading above the link group, e.g. "Blog — peterattiamd.com", "Podcast — The Drive #213"). The `source` field groups links by origin so readers know where a link leads before clicking.

```json
"expertCitations": [{
  "expert": "Dr. Peter Attia, MD",
  "work": "Outlive: The Science and Art of Longevity — Chapter 7 (2023)",
  "publicUrls": [
    { "source": "Blog — peterattiamd.com", "url": "https://...", "label": "Article title" },
    { "source": "Podcast — The Drive", "url": "https://...", "label": "Episode title" }
  ]
}]
```
...
1. Add entry to `CATEGORIES` object in `questions.json`, run `build.py`
2. Create `evidence-pages/{slug}.json`, run `evidence-builder.py {slug}`
